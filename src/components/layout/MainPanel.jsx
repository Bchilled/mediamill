    ...
    <div style={{flex:1,overflowY:'auto',padding:32}}>
      <div style={{maxWidth:800,margin:'0 auto'}}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:24}}>
          <div><h2 style={{fontSize:20,fontWeight:900,color:text,marginBottom:4}}>Channels</h2>
          <div style={{fontSize:12,color:muted}}>{(channels||[]).length} channel{(channels||[]).length!==1?'s':''}</div></div>
        ...
        {(channels||[]).length===0?(
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))',gap:16}}>
            {(channels||[]).map(ch=>{
              const isActive=activeChannel?.id===ch.id;const color=PC[ch.preset]||'#888';
              return(<div key={ch.id} onClick={()=>{setActiveChannel(ch);setActiveView('dashboard');}}
